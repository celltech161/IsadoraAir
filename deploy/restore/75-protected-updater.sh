...
# 90-system-config.sh's own established USE_SUDO idiom (`sudo
# restore_manage_command's resolved argv ...`, see lib.sh) -- never
# relaxes ownership/mode checks, never chowns after the fact; ownership
# simply falls out of whichever effective UID performs the copy. As with
# every other stage, restore_phase_d_component itself always comes from
...
if [ "$RESTORE_MODE" != "apply" ]; then
  ...
  log_plan "restore_manage restore_phase_d_component --recovery-payload <payload> --fake-root <tmp> --publish-root $PUBLISH_ROOT"
...
# requirements.txt) with one shared, clear diagnostic -- this stage only
# needs its own ordering precondition: has 20-application.sh actually
# reconstructed the target checkout yet.
...
# sudo process, but a resolved argv is.
restore_manage_command restore_phase_d_component \
