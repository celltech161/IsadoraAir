...
State vocabulary (`DiagnosticFact.state`):
    ...
    not_applicable    -- deliberate 5th state, added in Pass B: evidence
                          for an EVENT-DRIVEN artifact (the alert beep)
...
def get_weather_diagnostics(now=None, data_dir=None, library_root=None) -> WeatherDiagnosticsSnapshot:
    ...
    facts.append(_check_watch_warnings(data_dir / "active_watches_warnings.json", now))
    facts.append(_check_amber_alerts_data(amber_cfg, data_dir / "active_amber_alerts.json", now))

    for suffix, category_code, filename, event_driven in GENERATED_ARTIFACTS:
        facts.append(_check_generated_artifact(suffix, category_code, filename, event_driven, library_root, now))
...
def _check_station_location(cfg: WeatherConfig) -> DiagnosticFact:
    problems = []
...
def _check_nws_config(cfg: WeatherConfig) -> DiagnosticFact:
...
def _check_announcer_schedule(cfg: WeatherConfig) -> list[DiagnosticFact]:
...
def _check_alert_fx_cart(cfg: WeatherConfig) -> DiagnosticFact:
...
def _check_notifications(cfg: WeatherConfig) -> DiagnosticFact:
...
def _check_amber_config(cfg: AmberAlertConfig) -> DiagnosticFact:
    if not cfg.enabled:
        return DiagnosticFact(
...
def _check_simple_json_file(path: Path) -> DiagnosticFact:
    ...
    timestamp, age = _mtime_evidence(path, dj_timezone.now())
    return DiagnosticFact(
        key=key, state="ready",
        summary=f"{path.name} exists and parses.", path=str(path),
        timestamp=timestamp, age_seconds=age,
        evidence={"timestamp_source": "file_mtime"},
    )
...
def _check_latest_weather(path: Path, now) -> DiagnosticFact:
    ...
    data, error = _load_json_file(path)
    if error == "missing":
        return DiagnosticFact(key=key, state="degraded", summary="latest_weather.json does not exist yet.", path=str(path))
    if error:
        return DiagnosticFact(key=key, state="needs_attention", summary="latest_weather.json exists but failed to parse.", detail=error, path=str(path))
    ...
    if ts is not None:
        return DiagnosticFact(
            ...
            evidence={"timestamp_source": "payload.timestamp"},
        )
...
def _check_wind_history(path: Path, now) -> DiagnosticFact:
    ...
    data, error = _load_json_file(path)
    if error == "missing":
        return DiagnosticFact(key=key, state="degraded", summary="wind_history.json does not exist yet.", path=str(path))
    if error:
        return DiagnosticFact(key=key, state="needs_attention", summary="wind_history.json exists but failed to parse.", detail=error, path=str(path))
    if not isinstance(data, list):
        return DiagnosticFact(key=key, state="needs_attention", summary="wind_history.json is not a list.", path=str(path))
    latest_ts = None
    for entry in data:
...
def _check_smoothed_wind(path: Path, now) -> DiagnosticFact:
    ...
    data, error = _load_json_file(path)
    if error == "missing":
        return DiagnosticFact(key=key, state="degraded", summary="smoothed_wind.json does not exist yet.", path=str(path))
    if error:
        return DiagnosticFact(key=key, state="needs_attention", summary="smoothed_wind.json exists but failed to parse.", detail=error, path=str(path))
    ...
    if ts is not None:
        return DiagnosticFact(
            ...
            path=str(path), timestamp=_iso(ts), age_seconds=_age_seconds(now, ts),
            evidence={"timestamp_source": "payload.time"},
        )
    timestamp, age = _mtime_evidence(path, now)
    return DiagnosticFact(
...
def _check_forecast_cache(path: Path, now) -> DiagnosticFact:
    ...
    if error == "missing":
        ...
        # `needs_attention`).
        return DiagnosticFact(key=key, state="degraded", summary="No forecast cache exists yet.", path=str(path))
    if error:
        return DiagnosticFact(key=key, state="needs_attention", summary="Forecast cache exists but failed to parse.", detail=error, path=str(path))
    if not isinstance(data, list) or not data:
        return DiagnosticFact(key=key, state="needs_attention", summary="Forecast cache is not a non-empty list of periods.", path=str(path))

    timestamp, age = _mtime_evidence(path, now)
    age_hours = (age / 3600.0) if age is not None else None
    evidence = {"period_count": len(data), "timestamp_source": "file_mtime"}
...
def _check_watch_warnings(path: Path, now) -> DiagnosticFact:
    ...
    if error == "missing":
        ...
        # `needs_attention`.
        return DiagnosticFact(key=key, state="degraded", summary="No watch/warning snapshot exists yet -- pipeline has not completed a cycle.", path=str(path))
    if error:
        return DiagnosticFact(key=key, state="needs_attention", summary="Watch/warning snapshot exists but failed to parse.", detail=error, path=str(path))
    if not isinstance(data, list):
        return DiagnosticFact(key=key, state="needs_attention", summary="Watch/warning snapshot is not a list.", path=str(path))
    ...
    return DiagnosticFact(
        ...
        path=str(path), count=len(data), timestamp=timestamp, age_seconds=age,
        evidence={"events": events, "timestamp_source": "file_mtime"},
    )


def _check_amber_alerts_data(amber_cfg: AmberAlertConfig, path: Path, now) -> DiagnosticFact:
    key = "amber_alerts_data"
    if not amber_cfg.enabled:
        return DiagnosticFact(
    ...
    data, error = _load_json_file(path)
    if error == "missing":
        return DiagnosticFact(key=key, state="degraded", summary="AMBER-family alerts enabled but no snapshot exists yet.", path=str(path))
    if error:
        return DiagnosticFact(key=key, state="needs_attention", summary="AMBER-family alert snapshot exists but failed to parse.", detail=error, path=str(path))
    if not isinstance(data, list):
        return DiagnosticFact(key=key, state="needs_attention", summary="AMBER-family alert snapshot is not a list.", path=str(path))
    events = [e.get("event") for e in data if isinstance(e, dict) and e.get("event")]
    timestamp, age = _mtime_evidence(path, now)
    return DiagnosticFact(
        key=key, state="ready",
...
def _check_generated_artifact(suffix, category_code, filename, event_driven, library_root: Path, now) -> DiagnosticFact:
    ...
    evidence = {"category_code": category_code, "filename": filename, "event_driven": event_driven}
    timestamp = age = None
    if file_exists:
    ...
    if not file_exists and track is None:
        if event_driven:
            return DiagnosticFact(
                key=key, state="not_applicable",
                summary=f"No active alert -- {filename} not currently expected.",
    ...
    if not file_exists and track is not None:
        return DiagnosticFact(
            ...
            path=str(expected_path), evidence={**evidence, "track_id": track.id},
        )
    if not track.ready2air:
        return DiagnosticFact(
            ...
            evidence={**evidence, "track_id": track.id},
        )
    return DiagnosticFact(
        key=key, state="ready",
