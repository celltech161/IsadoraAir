...
| `ENGINE_COMMAND_QUEUE_DIR = Path("/run/isadoraair/engine_cmd.d")` / `ENGINE_COMMAND_LOCK_PATH = Path("/run/isadoraair/engine_cmd.lock")` | 2 constants in the in-tree shared `isadoraair.engine_commands` transport | Required production constants | `/run/isadoraair` is IsadoraAir's own `systemd-tmpfiles.d`-managed runtime namespace. All release-managed producers use this bounded queue; runtime creation is idempotent, so no additional service or tmpfiles entry is required. |
| `CMD_PATH = Path("/run/isadoraair/engine_cmd.json")` | 1 in-tree engine compatibility consumer; 1 external writer in `ogremote-ingest/lib/engine_cmd.py` | Required transition compatibility | The in-tree Weather/AMBER writers moved to the bounded queue in P2 1.13D. The separately versioned/deployed OGRemote companion remains a legacy single-slot writer, so the engine continues to consume this path temporarily. Multiple legacy writes between polls remain inherently last-write-wins. |
| `NOW_PLAYING_PATH = Path("/run/isadoraair/now_playing.json")` | 1 (syndicated-ingest `bsky/post_now_playing.py`) | Required production constant | Same `/run/isadoraair` runtime-namespace reasoning as the engine-command paths. |
| `KOKORO_BINARY = "/home/jreed/kokoro/bin/kokoro_synth"` | 1 (weather-ingest `lib/voices.py`) | Documentation-only issue / deferred | Genuinely user-specific today. Deliberately **not** touched in this pass -- see `docs/KOKORO_PROVENANCE.md`'s own recommendation; this is exactly the kind of "generic TTS runtime path" question that phase's dedicated inspection covers, and touching it here without that context risked a shallower fix. **Update (2026-08-30):** this deferred item is now addressed by the shared-TTS migration -- `KOKORO_BINARY` and all direct dispatch are removed from weather-ingest's `lib/voices.py` on its `feature/shared-tts-cutover` development branch, replaced by the canonical `/usr/local/bin/isadoraair-tts` CLI. Ready for production acceptance but not yet merged/deployed; `/home/jreed/kokoro` remains in place pending that acceptance. |
| `PIPER_BINARY = "/home/jreed/weather-ingest/venv/bin/piper"` | 2 (weather-ingest `lib/voices.py`, `wx_alert.py` -- duplicated, not shared) | Documentation-only issue / deferred | Same reasoning; see `docs/PIPER_PROVENANCE.md`. The duplication between `lib/voices.py` and `wx_alert.py` (two independent copies of the same constant) is itself worth a future small cleanup, noted here rather than fixed. **Update (2026-08-30):** both copies of `PIPER_BINARY` are removed on the same `feature/shared-tts-cutover` branch, for the same reason -- resolving both the path-portability deferral and the duplication in one migration. Ready for production acceptance, not yet merged/deployed. This pass's Python-source migration does not touch the forecast systemd units (`deploy/wx-forecast-*.service` still pass explicit `--voice day`/`--voice night`) -- see `docs/TTS_RUNTIME.md` for why that separate unit-level change was deferred. |
